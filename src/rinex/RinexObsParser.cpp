...
void RinexObsParser::Parse(std::istream &input) {
  this->InitParser();
  constants::rinex::detail::for_each_line(
      input, [this](std::string line) { this->ParseLine(std::move(line)); });
}
...
void RinexObsParser::ReadEpochHeader(std::string line) {
  ...
  try {
    ...
    int epochFlag = util::astring::parseInt(
        line.substr(31, 1)); // 0: OK; 1: power failure between current and
                             // previous epoch; >1 Special Event
    ...
    this->_Epochs.emplace_back(
        ObservationEpoch{.time = epoch, .flag = epochFlag, .satellites{}});
...
void RinexObsParser::ReadEpochObservation(std::string line) {
  ...
  this->_Epochs.back().satellites.emplace_back(
      SatelliteObservation{.satellite = satellite, .signals{}});
  ...
  for (const auto &obsDef : _ObservationDefinitions.at(satellite.SVSystem())) {
    ...
    const auto indicator =
        [&](std::size_t offset,
            unsigned int maximum) -> std::optional<unsigned int> {
    ...
    const SignalId signal{obsDef.GetObservationBand(),
                          obsDef.GetObservationAttribute()};

    switch (obsDef.GetObservationType()) {
    case ObservationType::Code: // Pseudorange
    {
      double psuedorange = util::astring::parseDouble(data);
      this->_Epochs.back().satellites.back().signals[signal].CodeObservation =
          Rinex::Observation::Code{psuedorange, quality};
    ...
    case ObservationType::Phase: // Carrierphase
    {
      ...
      this->_Epochs.back().satellites.back().signals[signal].PhaseObservation =
          Rinex::Observation::CarrierPhase{cycles, quality};

      break;
    }
    case ObservationType::Doppler: {
      double doppler = util::astring::parseDouble(data);
      this->_Epochs.back().satellites.back().signals[signal].DopplerObservation =
          Rinex::Observation::Doppler{doppler, quality};

      break;
    }
    case ObservationType::RawSignalStrength: {
      double snr = util::astring::parseDouble(data);
      this->_Epochs.back().satellites.back().signals[signal].SnrObservation =
          Rinex::Observation::SignalStrength{snr, quality};
...
void RinexObsParser::ReadObservationTypes(std::string line) {
  // SYS / # / OBS TYPES definition is: A1 2X,I3 13(1X,A3) [SvSystem
  // (optional)][number of ObsTypes][Type,Band,Attribute]
  ...
  for (auto it = std::begin(line) + 7;
       it != std::end(line) && !std::isspace(*it); it += 4) {
    ...
    _ObservationDefinitions[svSystem].emplace_back(obsType, obsBand,
                                                   obsAttribute);
  }
}
...
void RinexObsParser::ParseLine(std::string line) {
  ...
  case RinexParserState::PARSE_HEADER: {
    ...
    } else if ((line.find(RINEX_VERSION_DEFINITION) !=
                std::string::npos)) // read rinex file version
    {
      std::string str = line.substr(0, 9);
      this->_Version = ltrim(str);
    } else if ((line.find(RINEX_APPROX_POSITION_DEFINITION) !=
                std::string::npos)) // approximate position
    {
    ...
    } else if ((line.find(RINEX_ANTENNA_DELTA_DEFINITION) !=
                std::string::npos)) // antenna phase center offset
    {
      double x = util::astring::parseDouble(line.substr(0, 14));
      double y = util::astring::parseDouble(line.substr(14, 14));
      double z = util::astring::parseDouble(line.substr(28, 14));
      ...
      _SignalStrengthUnit =
          first == std::string::npos
              ...
              : std::optional<std::string>{
                    unit.substr(first, unit.find_last_not_of(' ') - first + 1)};
    } else if ((line.find(RINEX_OBS_TYPE_DEFINITION) !=
                std::string::npos)) // read all observation types from rnx
                                    // header
    {
  ...
  case RinexParserState::PARSE_OBS_TYPES: {
    if ((line.find(RINEX_OBS_TYPE_DEFINITION) !=
         std::string::npos)) // read all observation types from rnx header
    {
      this->ReadObservationTypes(line);
    } else {
  ...
  case RinexParserState::PARSE_EPOCH: {
    ...
    if (line[0] == '>') {
      ...
      if (this->_Epochs.back().flag > 1) {
        _RinexParserState =
            RinexParserState::PARSE_HEADER; // TODO AA: currently no handling
                                            // for special events!
      }
    } else {
