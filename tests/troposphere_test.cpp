...
int main() {
  ...
  if (!zenith || !std::isfinite(zenith->Total()) ||
      std::abs(zenith->hydrostatic_m - 2.3094967381) > 1e-8 ||
      std::abs(zenith->wet_m - 0.1204034929) > 1e-8 ||
      std::abs(zenith->Total() - 2.4299002310) > 1e-8) {
    std::cerr << "Missing or unexpected sea-level zenith delay\n";
    return EXIT_FAILURE;
  }
  ...
  for (double degrees : {60.0, 30.0, 10.0, 5.0}) {
    ...
    if (!delay || !std::isfinite(delay->Total()) || delay->Total() <= previous ||
        delay->hydrostatic_m <= zenith->hydrostatic_m ||
        delay->wet_m <= zenith->wet_m) {
      std::cerr << "Delay must exist and increase as elevation decreases\n";
      return EXIT_FAILURE;
    }
  ...
  if (!low || !std::isfinite(low->Total()) ||
      std::abs(low->Total() - 4.838365) > 1e-5 ||
      low->hydrostatic_m / zenith->hydrostatic_m >=
          low->wet_m / zenith->wet_m) {
    std::cerr << "Missing or unexpected separate hydrostatic/wet mapping\n";
    return EXIT_FAILURE;
  }

  const double nan = std::numeric_limits<double>::quiet_NaN();
  for (const auto input :
       {troposphere::Input{0.0, 0.0, 0.0}, troposphere::Input{0.0, 0.0, -0.1},
        troposphere::Input{0.0, 0.0, pi}, troposphere::Input{pi, 0.0, 1.0},
        troposphere::Input{0.0, -101.0, 1.0},
        troposphere::Input{0.0, 10001.0, 1.0},
        troposphere::Input{nan, 0.0, 1.0}, troposphere::Input{0.0, nan, 1.0},
        troposphere::Input{0.0, 0.0, nan}}) {
    if (model(input).has_value()) {
